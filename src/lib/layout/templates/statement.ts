import type { StatementSlideData } from "@/lib/types";
...
export function layoutStatement(
  ...
  // Statement text
  const statementFontSize = 72;
  const maxW = 1400;
  const statementHeight = estimateTextHeight(
    slide.statement,
    statementFontSize,
    1.15,
    maxW,
  ...
  elements.push({
    kind: "text",
    id: "statement",
    rect: { x: (CANVAS_W - maxW) / 2, y, w: maxW, h: statementHeight },
