...
- `src/components/` holds the slide engine and UI; templates live in `src/components/templates/` (e.g., `CoverSlide.tsx`).
- `src/lib/` contains YAML loading and shared types (`loadPresentation.ts`, `types.ts`).
...
- Tests live in `src/**.test.ts(x)` (Vitest) and `e2e/*.spec.ts` (Playwright). Design notes are in `docs/`.
...
- Template components follow the `*Slide.tsx` naming pattern and are registered in `src/components/templates/index.ts`.
- Prefer the `@/*` path alias for `src/*` imports.

## Testing Guidelines
- Unit/integration tests use Vitest in `src/**/*.test.ts(x)`. Add or update tests when changing loaders, templates, or engine behavior.
- E2E tests use Playwright in `e2e/*.spec.ts` for navigation, layout, and image loading.
...
- Use conventional commits with scope and a bullet list body, ending with a co-author line.
  Example:
  `feat(templates): add quote layout`
  `- Add QuoteSlide template`
