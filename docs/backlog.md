...
~~**Box flex-column → auto-layout**~~ — Done. Flex-column routes through `resolveBoxWithLayout()` using placeholder→`resolveLayouts()`→map-back pattern. Full justify/align/gap support. Mixed absolute+flow positioning via `position: "absolute"` on `ElementBase` — absolute children opt out of flow, keep rects unchanged, z-order preserved. Spacer defaults to `flex: true` when no height given.
...
1. ~~Box flex-column delegates to `resolveLayouts()`~~ — Done.
